"""用于校验 muldata 业务场景脚本的模块。"""
# 流程：
# 1. 先清理动态 id 的 repo-muldata-*，再创建 person/int8/ram、PreFilter=false 仓库并等待 READY。
# 2. 写入 entity1-1~3、entity2-1~3、entity3-1~3，9 条实体均预期 HTTP 201。
# 3. POST /repositories/search：include 包含 ENTITY1_FEATURE 和 ENTITY2_FEATURE 两条 data，include_threshold=0.99，exclude_threshold=0.1，max_candidates=5。
# 4. 断言 HTTP 200、结果数不超过 5、所有 similarity >= 0.99、similarity 非递增，且结果必须包含 entity1-1/2/3 与 entity2-1/2。
# 5. 当前接口若返回“Only single include item is supported”，该 search 步骤判定失败。
# 6. 最后删除仓库并确认不存在。
from __future__ import annotations

import asyncio
import json
import os
from pathlib import Path
from typing import Any

if __package__ in {None, ""}:
    import sys

    repo_root = str(Path(__file__).resolve().parents[3])
    if repo_root not in sys.path:
        sys.path.insert(0, repo_root)

from API_TEST.business.search._scenario_runtime import cleanup_repo, create_entity, create_repo, failure_reason_or_body, record_step, search_query, wait_repo_ready
from API_TEST.business.search._scenario_support import describe_search_failure, extract_result_ids, finalize_search_summary
from API_TEST.business.support import build_business_runtime, extract_result_similarities, extract_search_results, is_nonincreasing, make_repo_id, open_business_apis


# 1
ENTITY3_FEATURE = ""
# 0.9
ENTITY2_FEATURE = ""
# 0.5
ENTITY1_FEATURE = ""

EXPECTED_RESULT_IDS = ["entity1-1", "entity1-2", "entity1-3", "entity2-1", "entity2-2"]


# 内部辅助函数，封装当前模块的局部逻辑。
def _missing_expected_ids(result_ids: list[str]) -> list[str]:
    return [entity_id for entity_id in EXPECTED_RESULT_IDS if entity_id not in result_ids]


# 内部辅助函数，封装当前模块的局部逻辑。
def _assert_results(payload: dict[str, Any], include_threshold: float, max_candidates: int) -> list[str]:
    results = extract_search_results(payload)
    if not results:
        raise AssertionError(f"expected results with threshold {include_threshold}, got empty results")
    if len(results) > max_candidates:
        raise AssertionError(f"expected <= {max_candidates} results, got {len(results)}")
    similarities = extract_result_similarities(results)
    if not similarities:
        raise AssertionError(f"expected similarity scores >= {include_threshold}")
    if not all(sim >= include_threshold for sim in similarities):
        raise AssertionError(f"expected all similarities >= {include_threshold}, got {similarities}")
    if not is_nonincreasing(similarities):
        raise AssertionError(f"expected nonincreasing similarities, got {similarities}")
    result_ids = extract_result_ids(results)
    missing_expected_ids = _missing_expected_ids(result_ids)
    if missing_expected_ids:
        raise AssertionError(
            f"expected result ids to include {EXPECTED_RESULT_IDS}, got {result_ids}; missing {missing_expected_ids}"
        )
    return result_ids


# 执行当前场景的核心流程。
async def run_scenario(*, repo_api, entity_api, search_api, output_root: Path | None = None) -> dict[str, object]:
    collector, store, summary_path, _ = build_business_runtime(Path(__file__), "search", "muldata", output_root)
    repo_id = make_repo_id("repo-muldata")
    include_threshold = 0.99
    max_candidates = 5
    overall_status = "passed"
    case_results: list[dict[str, object]] = []
    returned_result_ids: list[str] = []
    missing_expected_ids: list[str] = list(EXPECTED_RESULT_IDS)

    try:
        try:
            cleanup_before = await cleanup_repo(repo_api, store, repo_id=repo_id, name="cleanup muldata repo before run")
            record_step(collector, case_results, prefix="muldata", case_id="cleanup-before", expected_status="200/202/204/404", actual_status=cleanup_before.status_code, passed=True)
        except Exception as exc:
            overall_status = "failed"
            record_step(collector, case_results, prefix="muldata", case_id="cleanup-before-error", expected_status="cleanup ok", actual_status=None, passed=False, failure_reason=str(exc))

        created = await create_repo(repo_api, store, repo_id=repo_id, payload={"id": repo_id, "type": "person", "index_type": "int8", "level": "ram", "options": {"PreFilter": "false"}}, case_id="create", name="create muldata repo")
        create_ok = created.status_code in {200, 201, 202}
        if not create_ok:
            overall_status = "failed"
        create_reason = None if create_ok else failure_reason_or_body(created.response_body, f"expected 200/201/202, got {created.status_code}")
        record_step(collector, case_results, prefix="muldata", case_id="create", expected_status="200/201/202", actual_status=created.status_code, passed=create_ok, failure_reason=create_reason)

        if create_ok:
            ready = await wait_repo_ready(repo_api, store, repo_id=repo_id, timeout_s=30.0)
            ready_ok = ready.status_code == 200
            if not ready_ok:
                overall_status = "failed"
            ready_reason = None if ready_ok else failure_reason_or_body(ready.response_body, f"expected 200, got {ready.status_code}")
            record_step(collector, case_results, prefix="muldata", case_id="ready", expected_status=200, actual_status=ready.status_code, passed=ready_ok, failure_reason=ready_reason)

            if ready_ok:
                for idx in range(1, 4):
                    entity_record = await create_entity(entity_api, store, repo_id=repo_id, entity_id=f"entity1-{idx}", payload={"id": f"entity1-{idx}", "data": {"type": "feature", "value": ENTITY1_FEATURE}, "location_id": "1"}, case_id=f"entity1-{idx}", name=f"create entity1 {idx}")
                    entity_ok = entity_record.status_code == 201
                    if not entity_ok:
                        overall_status = "failed"
                    entity_reason = None if entity_ok else failure_reason_or_body(entity_record.response_body, f"expected 201, got {entity_record.status_code}")
                    record_step(collector, case_results, prefix="muldata", case_id=f"entity1-{idx}", expected_status=201, actual_status=entity_record.status_code, passed=entity_ok, failure_reason=entity_reason)

                for idx in range(1, 4):
                    entity_record = await create_entity(entity_api, store, repo_id=repo_id, entity_id=f"entity2-{idx}", payload={"id": f"entity2-{idx}", "data": {"type": "feature", "value": ENTITY2_FEATURE}, "location_id": "1"}, case_id=f"entity2-{idx}", name=f"create entity2 {idx}")
                    entity_ok = entity_record.status_code == 201
                    if not entity_ok:
                        overall_status = "failed"
                    entity_reason = None if entity_ok else failure_reason_or_body(entity_record.response_body, f"expected 201, got {entity_record.status_code}")
                    record_step(collector, case_results, prefix="muldata", case_id=f"entity2-{idx}", expected_status=201, actual_status=entity_record.status_code, passed=entity_ok, failure_reason=entity_reason)

                for idx in range(1, 4):
                    entity_record = await create_entity(entity_api, store, repo_id=repo_id, entity_id=f"entity3-{idx}", payload={"id": f"entity3-{idx}", "data": {"type": "feature", "value": ENTITY3_FEATURE}, "location_id": "1"}, case_id=f"entity3-{idx}", name=f"create entity3 {idx}")
                    entity_ok = entity_record.status_code == 201
                    if not entity_ok:
                        overall_status = "failed"
                    entity_reason = None if entity_ok else failure_reason_or_body(entity_record.response_body, f"expected 201, got {entity_record.status_code}")
                    record_step(collector, case_results, prefix="muldata", case_id=f"entity3-{idx}", expected_status=201, actual_status=entity_record.status_code, passed=entity_ok, failure_reason=entity_reason)

                payload = {
                    "type": "person",
                    "include": [
                        {"data": {"value": ENTITY1_FEATURE, "type": "feature"}},
                        {"data": {"value": ENTITY2_FEATURE, "type": "feature"}},
                    ],
                    "repositories": [repo_id],
                    "include_threshold": include_threshold,
                    "exclude_threshold": 0.1,
                    "max_candidates": max_candidates
                }
                search_record = await search_query(search_api, store, payload=payload, case_id="search", name="search muldata")
                search_ok = search_record.status_code == 200
                search_reason = None
                result_ids: list[str] = []
                if search_ok:
                    result_ids = extract_result_ids(extract_search_results(search_record.response_body))
                    returned_result_ids = result_ids
                    missing_expected_ids = _missing_expected_ids(result_ids)
                    try:
                        result_ids = _assert_results(search_record.response_body, include_threshold, max_candidates)
                    except Exception as exc:
                        search_ok = False
                        search_reason = f"{exc} (threshold={include_threshold})"
                else:
                    search_reason = describe_search_failure(search_record.response_body, f"expected 200, got {search_record.status_code}")
                if not search_ok:
                    overall_status = "failed"
                    search_reason = search_reason or describe_search_failure(search_record.response_body, "search validation failed")
                record_step(
                    collector,
                    case_results,
                    prefix="muldata",
                    case_id="search",
                    expected_status="200 + threshold constraints + entity1-1/2/3 and entity2-1/2 returned",
                    actual_status=search_record.status_code,
                    passed=search_ok,
                    failure_reason=search_reason,
                    extra={
                        "expected_result_ids": list(EXPECTED_RESULT_IDS),
                        "returned_result_ids": list(returned_result_ids),
                        "missing_expected_ids": list(missing_expected_ids),
                    },
                )
                if search_ok:
                    print("Muldata validation completed successfully.")
                    print(f"Returned ids: {result_ids}")
                    print(f"Threshold: {include_threshold}, max_candidates/topk: {max_candidates}")
    except Exception as exc:
        overall_status = "failed"
        record_step(collector, case_results, prefix="muldata", case_id="scenario-error", expected_status="no exception", actual_status=None, passed=False, failure_reason=str(exc))
    finally:
        try:
            cleanup_after = await cleanup_repo(repo_api, store, repo_id=repo_id, name="cleanup muldata repo after run")
            record_step(collector, case_results, prefix="muldata", case_id="cleanup-after", expected_status="200/202/204/404", actual_status=cleanup_after.status_code, passed=True)
        except Exception as exc:
            overall_status = "failed"
            record_step(collector, case_results, prefix="muldata", case_id="cleanup-after-error", expected_status="cleanup ok", actual_status=None, passed=False, failure_reason=str(exc))

        summary = finalize_search_summary(
            collector,
            summary_path,
            overall_status=overall_status,
            extra={
                "repo_id": repo_id,
                "case_total": len(case_results),
                "case_results": case_results,
                "expected_result_ids": list(EXPECTED_RESULT_IDS),
                "returned_result_ids": list(returned_result_ids),
                "missing_expected_ids": list(missing_expected_ids),
            },
        )
    return summary


# 程序入口，用于串起当前模块的执行流程。
async def main() -> None:
    async with open_business_apis(repo=True, entity=True, search=True) as apis:
        summary = await run_scenario(repo_api=apis["repo_api"], entity_api=apis["entity_api"], search_api=apis["search_api"])
        print(json.dumps(summary, ensure_ascii=False, indent=2))


if __name__ == "__main__":
    if os.environ.get("API_TEST_IMPORT_ONLY") == "1":
        raise SystemExit(0)
    asyncio.run(main())
