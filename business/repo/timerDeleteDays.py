"""用于校验仓库 TimerDeleteDays 配置的业务场景脚本。"""
# 流程：
# 1. 创建动态 id 的 business-timer-delete-days-repo-* 仓库，options.TimerDeleteDays=1，预期创建 202。
# 2. 校验仓库选项 TimerDeleteDays=1。
# 3. 使用顶层 id、data、location_id 字段创建 td-1 ~ td-3，3 次均预期 HTTP 201。
# 4. 本场景不验证跨天自动删除，只验证配置回显和实体写入链路。
...
async def _cleanup_repo(repo_api, store, *, repo_id: str) -> None:
    cleanup = await repo_api.delete(None, case_id="cleanup", module="repo", action="delete", name="cleanup timerDeleteDays repo", path=f"/repositories/{repo_id}")
    store.save(cleanup)
...
async def run_scenario(*, repo_api, entity_api, output_root: Path | None = None) -> dict[str, object]:
    config = load_business_config("repo", "timerDeleteDays")
    collector, store, summary_path, _ = build_business_runtime(Path(__file__), "repo", "timerDeleteDays", output_root)
    ...
    try:
        created = await repo_api.create(repo_payload, case_id="create", module="repo", action="create", name="create timerDeleteDays repo", path="/repositories")
        store.save(created)
        ...
        record_case_result(case_results, prefix="timerDeleteDays", case_id="create", expected_status=202, actual_status=created.status_code, passed=create_ok, failure_reason=None if create_ok else create_reason)
        ...
        option_ok = ready_ok and _repo_option(ready, "TimerDeleteDays") == "1"
        if option_ok:
            collector.record_success("option")
        else:
            overall_status = "failed"
            option_reason = f"expected TimerDeleteDays=1, got {_repo_option(ready, 'TimerDeleteDays')}"
            collector.record_failure("option", option_reason)
        record_case_result(case_results, prefix="timerDeleteDays", case_id=f"repo-ready-{repo_id}", expected_status="TimerDeleteDays=1", actual_status=_repo_option(ready, "TimerDeleteDays") if ready_ok else None, passed=option_ok, failure_reason=None if option_ok else option_reason)

        for idx in range(1, 4):
            entity_record = await _create_entity(entity_api, store, repo_id=repo_id, entity_id=f"td-{idx}", value=feature_value)
            ...
            record_case_result(case_results, prefix="timerDeleteDays", case_id=f"entity-create-td-{idx}", expected_status=201, actual_status=entity_record.status_code, passed=entity_ok, failure_reason=None if entity_ok else entity_reason)
    except Exception as exc:
        overall_status = "failed"
        collector.record_failure("timerDeleteDays", str(exc))
        record_case_result(case_results, prefix="timerDeleteDays", case_id="scenario-error", expected_status="no exception", actual_status=None, passed=False, failure_reason=str(exc))
    finally:
        try:
            ...
            record_case_result(case_results, prefix="timerDeleteDays", case_id="cleanup-error", expected_status="cleanup ok", actual_status=None, passed=False, failure_reason=str(exc))
